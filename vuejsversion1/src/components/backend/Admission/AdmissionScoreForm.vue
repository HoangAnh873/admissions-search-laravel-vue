...
const loadOptions = async () => {
  try {
    ...
    console.log('Dữ liệu khoa:', khoaRes)
    console.log('Dữ liệu ngành:', nganhRes)
    console.log('Dữ liệu năm:', namRes)
    console.log('Dữ liệu phương thức:', phuongThucRes)

    if (Array.isArray(khoaRes)) {
      khoas.value = khoaRes
      console.log('Đã load danh sách khoa:', khoas.value)
    } else {
      console.error('Dữ liệu khoa không đúng định dạng:', khoaRes)
      store.dispatch('toast/error', 'Dữ liệu khoa không đúng định dạng')
    }
    ...
    if (Array.isArray(phuongThucRes)) {
    ...
    store.dispatch('toast/error', `Không thể tải dữ liệu cho form: ${error.message}`)
  }
}
...
watch(() => props.editData, async (newVal) => {
  console.log('Edit data received in form:', JSON.stringify(newVal, null, 2))
  if (newVal) {
    ...
    try {
      ...
      formData.value = {
        ...
        chiTieu: newVal.chiTieu || '',
        diemChuan: newVal.diemChuan || '',
...
const modalTitle = computed(() => {
  return props.editData ? 'Sửa điểm chuẩn' : 'Thêm điểm chuẩn mới'
})
...
const validateField = (field) => {
  ...
  switch (field) {
    ...
    case 'phuong_thuc_id':
      ...
      break
    case 'chiTieu':
      if (!formData.value.chiTieu) {
        errors.value.chiTieu = 'Vui lòng nhập chỉ tiêu'
      } else if (formData.value.chiTieu < 0) {
        errors.value.chiTieu = 'Chỉ tiêu không được âm'
      }
      break
    case 'diemChuan':
      if (!formData.value.diemChuan) {
        errors.value.diemChuan = 'Vui lòng nhập điểm chuẩn'
      } else if (formData.value.diemChuan < 0) {
        errors.value.diemChuan = 'Điểm chuẩn không được âm'
      } else if (formData.value.diemChuan > 30) {
        errors.value.diemChuan = 'Điểm chuẩn không được vượt quá 30'
      }
      break
  }
}
...
watch(() => formData.value.chiTieu, () => validateField('chiTieu'))
watch(() => formData.value.diemChuan, () => validateField('diemChuan'))

const validateForm = async () => {
  validateField('nganh_id')
  validateField('nam_id')
  validateField('phuong_thuc_id')
  validateField('chiTieu')
  validateField('diemChuan')
  
  // Kiểm tra trùng lặp dữ liệu
  if (!errors.value.nganh_id && !errors.value.nam_id && !errors.value.phuong_thuc_id) {
    try {
      const response = await axios.get('/api/lich-su-diems/check-duplicate', {
        params: {
      ...
      if (response.exists) {
        errors.value.nganh_id = 'Đã tồn tại điểm chuẩn cho ngành này trong năm và phương thức xét tuyển đã chọn'
        return false
      }
    } catch (error) {
...
const handleSubmit = async () => {
  try {
    ...
    const dataToSend = {
      ...
      chiTieu: parseInt(formData.value.chiTieu),
      diemChuan: parseFloat(formData.value.diemChuan),
    ...
    if (props.editData) {
      // Update
      const response = await axios.put(`/api/lich-su-diems/${props.editData.id}`, dataToSend)
      console.log('Update response:', response)
      store.dispatch('toast/success', 'Cập nhật điểm chuẩn thành công')
    } else {
      // Create
      const response = await axios.post('/api/lich-su-diems', dataToSend)
      console.log('Create response:', response)
      store.dispatch('toast/success', 'Thêm điểm chuẩn thành công')
    }
    ...
    if (error.response?.data?.errors) {
      ...
      store.dispatch('toast/error', `Không thể lưu điểm chuẩn: ${error.response?.data?.message || error.message}`)
    }
  } finally {
    loading.value = false
  }
  ...
  <div v-if="show" class="uk-modal uk-open" style="display: block;">
    <div class="uk-modal-dialog uk-modal-body">
      ...
      <form v-else @submit.prevent="handleSubmit">
        ...
        <div class="uk-margin">
          <label class="uk-form-label">Chỉ tiêu <span class="uk-text-danger">*</span></label>
          <div class="uk-form-controls">
            <input 
              type="number" 
              v-model="formData.chiTieu" 
              class="uk-input" 
              :class="{ 'uk-form-danger': errors.chiTieu }"
              placeholder="Nhập chỉ tiêu"
              @blur="validateField('chiTieu')"
            >
            <transition name="fade">
              <div v-if="errors.chiTieu" class="error-message">{{ errors.chiTieu }}</div>
            </transition>
          </div>
        </div>

        <div class="uk-margin">
          <label class="uk-form-label">Điểm chuẩn <span class="uk-text-danger">*</span></label>
          <div class="uk-form-controls">
            <input 
              type="number" 
              v-model="formData.diemChuan" 
              class="uk-input" 
              :class="{ 'uk-form-danger': errors.diemChuan }"
              placeholder="Nhập điểm chuẩn"
              step="0.01"
              min="0"
              max="30"
              @blur="validateField('diemChuan')"
            >
            <transition name="fade">
              <div v-if="errors.diemChuan" class="error-message">{{ errors.diemChuan }}</div>
            </transition>
          </div>
        </div>

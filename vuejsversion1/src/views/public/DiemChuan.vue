<template>
  <public-layout>
    <div class="page-container">
      <h1 class="page-title">Điểm chuẩn các năm</h1>
      
      <div class="diem-chuan-content">
        ...
        <div v-else class="no-data">Không có dữ liệu điểm chuẩn</div>
      </div>
    </div>
  </public-layout>
</template>
...
export default {
  ...
  methods: {
    ...
    async fetchPhuongThuc() {
      try {
        ...
        console.error('Lỗi khi tải dữ liệu phương thức:', error)
    ...
    async fetchDiemChuan() {
      try {
        ...
        if (this.selectedMethod) {
          params.phuong_thuc_id = this.selectedMethod
        }
        ...
        if (response && response.success) {
          ...
          // Cập nhật lại dữ liệu điểm chuẩn cho các ngành
          if (this.faculties.length > 0) {
            this.faculties.forEach(faculty => {
              faculty.nganh.forEach(major => {
                major.diemChuan = this.diemChuan
                  .filter(dc => dc.nganh_id === major.id)
                  .map(score => ({
                    nam: score.nam,
  ...
  async created() {
    await Promise.all([
      this.fetchFaculties(),
      this.fetchYears(),
      this.fetchPhuongThuc()
    ])
    // Không chọn mặc định năm hay phương thức
    this.selectedYear = ''
    this.selectedMethod = ''

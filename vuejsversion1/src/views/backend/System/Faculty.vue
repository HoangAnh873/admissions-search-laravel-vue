...
import FacultyForm from '@/components/backend/System/FacultyForm.vue'
...
const filteredList = computed(() => {
  return list.value.filter(item => {
    ...
    const matchSearch = !filters.value.search || item.tenKhoa.toLowerCase().includes(filters.value.search.toLowerCase())
...
const loadData = async () => {
  ...
  try {
    ...
    if (response) {
      ...
      console.log('Danh sách khoa sau khi xử lý:', list.value)
    } else {
    ...
    store.dispatch('toast/error', `Không thể tải danh sách khoa: ${error.response?.data?.message || error.message}`)
    list.value = []
  } finally {
    loading.value = false
  }
...
const handleDelete = async (item) => {
  try {
    const confirmMessage = `Bạn có chắc chắn muốn xóa khoa này?\n\n` +
    ...
    if (confirm(confirmMessage)) {
      ...
      console.log('Đang xóa khoa:', item.id)
      const response = await axios.delete(`/api/khoas/${item.id}`)
      console.log('Response sau khi xóa:', response)
      
      store.dispatch('toast/success', 'Xóa khoa thành công')
...
const handleBulkDelete = async () => {
  ...
  const confirmMessage = `Bạn có chắc chắn muốn xóa ${selectedItems.value.length} khoa đã chọn?`
  ...
  if (confirm(confirmMessage)) {
    ...
    try {
      ...
      if (failedCount > 0) {
        ...
        store.dispatch('toast/error', `Không thể xóa ${failedCount} khoa do có dữ liệu ngành liên quan`)
      }
      
      if (successCount > 0) {
        store.dispatch('toast/success', `Đã xóa thành công ${successCount} khoa`)
      }
      ...
      store.dispatch('toast/error', 'Không thể xóa một số khoa do có dữ liệu ngành liên quan')
    } finally {
      loading.value = false
    }
  }
          ...
          <div class="uk-container">
            <div class="header-content">
              <div class="header-left">
                <h1 class="dashboard-title">Quản lý khoa</h1>
                <p class="uk-text-muted">Danh sách các khoa</p>
              </div>
          ...
          <div class="uk-card uk-card-default">
            <div class="uk-card-header">
              <div class="uk-grid-small" uk-grid>
                ...
                <div class="uk-width-auto">
                  <div class="uk-flex uk-flex-middle">
                    <div class="uk-search uk-search-default uk-margin-small-right">
            ...
            <div class="uk-card-body">
              ...
              <div v-else-if="list.length === 0" class="uk-text-center uk-padding">
                <p class="uk-text-muted">Không có dữ liệu khoa</p>
              </div>

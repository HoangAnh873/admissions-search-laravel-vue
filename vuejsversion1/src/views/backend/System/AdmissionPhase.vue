...
import AdmissionPhaseForm from '@/components/backend/System/AdmissionPhaseForm.vue'
...
const filteredList = computed(() => {
  return list.value.filter(item => {
    ...
    const matchSearch = !filters.value.search || item.tenDot.toLowerCase().includes(filters.value.search.toLowerCase())
...
const loadData = async () => {
  loading.value = true
  try {
    console.log('Đang tải danh sách giai đoạn xét tuyển...')
    const response = await axios.get('/api/giai-doan-tuyen-sinhs')
    ...
    if (response) {
      ...
      console.log('Danh sách giai đoạn xét tuyển sau khi xử lý:', list.value)
    } else {
      console.warn('Không có dữ liệu giai đoạn xét tuyển trong response:', response)
      list.value = []
    }
  } catch (error) {
    console.error('Lỗi khi tải danh sách:', error)
    store.dispatch('toast/error', `Không thể tải danh sách giai đoạn xét tuyển: ${error.response?.data?.message || error.message}`)
    list.value = []
  } finally {
    loading.value = false
  }
...
const handleDelete = async (item) => {
  try {
    const confirmMessage = `Bạn có chắc chắn muốn xóa giai đoạn xét tuyển này?\n\n` +
      `Tên giai đoạn: ${item.tenGiaiDoan}`

    if (confirm(confirmMessage)) {
      loading.value = true
      await csrf.getCookie()
      
      console.log('Đang xóa giai đoạn xét tuyển:', item.id)
      const response = await axios.delete(`/api/giai-doan-tuyen-sinhs/${item.id}`)
      console.log('Response sau khi xóa:', response)
      
      store.dispatch('toast/success', 'Xóa giai đoạn xét tuyển thành công')
      await loadData() // Tải lại danh sách sau khi xóa
    }
  } catch (error) {
    console.error('Lỗi khi xóa giai đoạn xét tuyển:', error)
    const errorMessage = error.response?.data?.message || error.message
    store.dispatch('toast/error', `Không thể xóa giai đoạn xét tuyển: ${errorMessage}`)
  } finally {
    loading.value = false
  }
}
...
const handleBulkDelete = async () => {
  ...
  const confirmMessage = `Bạn có chắc chắn muốn xóa ${selectedItems.value.length} đợt đã chọn?`
  
  if (confirm(confirmMessage)) {
    loading.value = true
    try {
      await csrf.getCookie()
      await Promise.all(selectedItems.value.map(id => axios.delete(`/api/dot-xet-tuyens/${id}`)))
      store.dispatch('toast/success', 'Xóa đợt thành công')
      selectedItems.value = []
      selectAll.value = false
      await loadData()
    } catch (error) {
      console.error('Lỗi khi xóa hàng loạt:', error)
      store.dispatch('toast/error', 'Không thể xóa một số đợt')
    } finally {
      loading.value = false
    }
  }
      ...
      <div class="admission-page">
        <div class="dashboard-header">
          <div class="uk-container">
            <h1 class="dashboard-title">Danh sách giai đoạn xét tuyển</h1>
            <p class="uk-text-muted">Quản lý thông tin các giai đoạn xét tuyển</p>
          </div>
        </div>
          ...
          <div class="uk-card uk-card-default">
            ...
            <div class="uk-card-body">
              ...
              <div v-else-if="list.length === 0" class="uk-text-center uk-padding">
                <p class="uk-text-muted">Không có dữ liệu giai đoạn xét tuyển</p>
              </div>
                ...
                <table class="uk-table uk-table-hover uk-table-middle uk-table-divider fixed-table">
                  <thead>
                    <tr>
                      ...
                      <th class="uk-width-4-5">Giai đoạn xét tuyển</th>
